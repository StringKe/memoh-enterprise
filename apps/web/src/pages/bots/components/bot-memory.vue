  ...
  <div class="flex gap-6 h-full absolute inset-0 p-4 mx-auto">
    ...
    <div class="w-64 shrink-0 flex flex-col border rounded-lg overflow-hidden max-h-full">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-1">
            ...
            <Button
              ...
              @click="loadMemories"
            >
              <RefreshCw :class="{ 'animate-spin': loading }" class="size-3.5" />
            </Button>
          </div>
        </div>
        <div class="relative">
      ...
      <ScrollArea class="flex-1 min-h-0">
        <div class="p-2 space-y-1">
          <div v-if="loading && memories.length === 0" class="p-4 text-center">
          ...
          >
            ...
            <div
              class="mt-1 text-[10px] text-muted-foreground truncate opacity-70 group-hover:opacity-100"
            >
      ...
      <div class="p-2 border-t mt-auto">
        <Button variant="outline" size="sm" class="w-full h-8 text-xs" @click="openNewMemoryDialog">
    ...
    <div class="flex-1 flex flex-col border rounded-lg overflow-hidden">
      <template v-if="selectedMemory">
        <div class="flex-1 flex flex-col min-h-0">
          <div class="p-3 border-b flex items-center justify-between bg-muted/30 shrink-0">
            ...
            <div class="flex items-center gap-2 shrink-0">
              <ConfirmPopover :message="$t('bots.memory.deleteConfirm')" @confirm="handleDelete">
                <template #trigger>
                  <Button
                    variant="ghost"
                    size="sm"
        ...
        <!-- Charts Section -->
        <div v-if="showChartSection" class="h-60 border-t flex flex-col bg-muted/5 shrink-0">
          ...
          <div class="flex-1 flex min-h-0 divide-x overflow-hidden">
            ...
            <div class="flex-1 flex flex-col p-3 min-w-0">
              ...
              <VChart class="h-full w-full min-h-0" :option="chartLeftOption" autoresize />
            </div>
            ...
            <div class="flex-1 flex flex-col p-3 min-w-0">
              ...
              <VChart class="h-full w-full min-h-0" :option="chartRightOption" autoresize />
            </div>
          </div>
        </div>
      </template>
      ...
      >
        <div class="size-12 rounded-full bg-muted flex items-center justify-center mb-4">
          <Brain class="size-6 opacity-20" />
        </div>
        <h3 class="text-xs font-medium text-foreground">
          {{ $t("bots.memory.title") }}
        </h3>
        <p class="text-xs mt-1 max-w-60">
          Select a file from the sidebar to view or edit, or create a new one to persist long-term
          information for your bot.
        </p>
        <Button variant="outline" size="sm" class="mt-6" @click="openNewMemoryDialog">
          {{ $t("bots.memory.newMemory") }}
        </Button>
      </div>
    </div>
      ...
      <DialogContent class="sm:max-w-2xl max-h-[80vh] flex flex-col">
        ...
        <div class="flex-1 min-h-0 overflow-hidden flex flex-col gap-4 py-4">
          <div class="flex items-center gap-4 shrink-0">
            <Button variant="outline" size="sm" class="text-xs h-8" @click="loadHistory">
              <RefreshCw :class="{ 'animate-spin': historyLoading }" class="mr-1.5 size-3" />
            ...
            <div class="space-y-2">
              ...
              >
                <div
                  ...
                  :class="
                    selectedHistoryMessages.includes(msg)
                      ? 'bg-primary text-primary-foreground'
                      : 'bg-background'
                  "
                >
                  <Check v-if="selectedHistoryMessages.includes(msg)" class="size-2.5" />
                </div>
                <div class="min-w-0">
                  <Badge variant="outline" class="text-[9px] uppercase px-1 py-0 h-3.5 mb-1">
                    {{ msg.role }}
                  </Badge>
        ...
        <DialogFooter>
          <Button variant="outline" @click="newMemoryDialogOpen = false">
            {{ $t("common.cancel") }}
          </Button>
          <Button :disabled="actionLoading || !newMemoryContent.trim()" @click="handleCreateMemory">
            <Spinner v-if="actionLoading" class="mr-1.5 size-3" />
            {{ $t("common.confirm") }}
          </Button>
        </DialogFooter>
      ...
      <DialogContent class="sm:max-w-md">
        ...
        <div class="py-4 space-y-6">
          ...
          <div class="space-y-3">
            ...
            <RadioGroup v-model="compactRatio" class="grid grid-cols-1 gap-3">
              <Label
          ...
          <div class="space-y-3">
            ...
            <Input v-model="compactDecayDate" type="date" class="w-full" />
        ...
        <DialogFooter>
          <Button variant="outline" @click="compactDialogOpen = false">
            {{ $t("common.cancel") }}
          </Button>
          <Button :disabled="compactLoading" @click="handleCompact">
...
const selectedDisplayCdfCurve = computed(() => buildDisplayCdfCurve(selectedCdfCurve.value, 48));
const topKBucketValues = computed(() =>
  selectedTopKBuckets.value.map((bucket: MemoryTopKBucket) => bucket.value ?? 0),
);
const topKMinValue = computed(() =>
  topKBucketValues.value.length > 0 ? Math.min(...topKBucketValues.value) : 0,
);
const topKMaxValue = computed(() =>
  topKBucketValues.value.length > 0 ? Math.max(...topKBucketValues.value) : 0,
);
const denseScores = computed(() => denseSearchResults.value.map((item) => item.score));
const denseScoreMax = computed(() =>
  denseScores.value.length > 0 ? Math.max(...denseScores.value) : 1,
);
...
const chartPalette = computed(() => {
  ...
  return {
    ...
    splitLine: resolveCssColor(
      "color-mix(in oklab, var(--muted-foreground) 8%, transparent)",
      "rgba(107,114,128,0.12)",
    ),
    topKBar: resolveCssColor(
      "color-mix(in oklab, var(--primary) 16%, transparent)",
      "rgba(99,102,241,0.18)",
    ),
    topKBarHover: resolveCssColor(
      "color-mix(in oklab, var(--primary) 26%, transparent)",
      "rgba(99,102,241,0.26)",
    ),
    cdfLine: resolveCssColor(
      "color-mix(in oklab, var(--primary) 34%, var(--foreground) 12%)",
      "rgba(99,102,241,0.46)",
    ),
    cdfArea: resolveCssColor(
      "color-mix(in oklab, var(--primary) 8%, transparent)",
      "rgba(99,102,241,0.09)",
    ),
    cdfPointer: resolveCssColor(
...
const chartLeftTitle = computed(() => (isDenseMode.value ? "Top-K Similarity" : "Top-K Bucket"));
...
function buildCdfSamplingTargets(maxPoints: number) {
  ...
  return Array.from(new Set(targets.map((target) => Number(target.toFixed(6))))).sort(
    (a, b) => a - b,
  );
}
