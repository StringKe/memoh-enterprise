<template>
  <div class="space-y-1.5">
    <div v-if="tasks.length && !results.length" class="space-y-1">
      <div v-for="(task, idx) in tasks" :key="idx" class="flex items-start gap-1.5 text-xs">
        <span class="font-mono text-foreground shrink-0">#{{ idx + 1 }}</span>
        <span class="truncate text-muted-foreground" :title="task">{{ task }}</span>
      </div>
    </div>
    ...
    <div v-if="results.length" class="space-y-1">
      ...
      >
        <CircleCheck v-if="result.success" class="size-3 text-emerald-500 shrink-0" />
        <CircleX v-else class="size-3 text-destructive shrink-0" />
        <span class="font-mono text-foreground shrink-0">#{{ idx + 1 }}</span>
        <span v-if="result.task" class="truncate text-muted-foreground" :title="result.task">{{
          result.task
        }}</span>
        <ExternalLink
    ...
    <div v-if="hasDetailedResults" class="space-y-1 pt-1 border-t border-border/50">
      <div v-for="(result, idx) in results" :key="idx">
        <pre
          ...
          >{{ result.text }}</pre
        >
        <p v-if="result.error" class="text-xs text-destructive">
          {{ result.error }}
        </p>
      </div>
    </div>

    <p v-if="!tasks.length && !results.length" class="text-xs text-muted-foreground italic">
...
const hasDetailedResults = computed(() => results.value.some((r) => r.text || r.error));
