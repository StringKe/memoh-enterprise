...
<template>
  <MasterDetailSidebarLayout class="[&_td:last-child]:w-45">
    <template #sidebar-content>
      <SidebarMenu v-for="providerItem in curFilterProvider" :key="providerItem.name">
        <SidebarMenuItem>
            ...
            >
              <span class="relative shrink-0">
                <span class="flex size-7 items-center justify-center rounded-full bg-muted">
                  <ProviderIcon v-if="providerItem.icon" :icon="providerItem.icon" size="1.25em" />
    ...
    <template #detail>
      <ScrollArea v-if="curProvider?.id" class="max-h-full h-full">
        <model-setting />
      </ScrollArea>
      <Empty v-else class="h-full flex justify-center items-center">
        ...
        <EmptyContent>
          <Button variant="outline" @click="openStatus.provideOpen = true">
            {{ $t("provider.addBtn") }}
          </Button>
        </EmptyContent>
      </Empty>
