<template>
  <div class="space-y-1.5">
    <div v-if="format" class="text-[10px] uppercase tracking-wide text-muted-foreground/70">
      {{ format }}
    </div>
    <div v-if="title" class="text-xs font-medium text-foreground">
      {{ title }}
    </div>
    <div v-if="excerpt" class="text-[11px] text-muted-foreground italic">
      {{ excerpt }}
    </div>
    <pre
      v-if="contentPreview"
