  ...
  <Popover v-model:open="open">
    ...
    <PopoverContent class="w-[--reka-popover-trigger-width] p-0" align="start">
      ...
      <div class="max-h-64 overflow-y-auto" role="listbox">
        ...
        <div v-for="group in filteredGroups" :key="group.key" class="p-1 *:mt-1 overflow-hidden">
          <div
          ...
          >
            <Check v-if="selected === option.value" class="size-3.5 shrink-0" />
            <span v-else class="size-3.5 shrink-0" />
            <slot name="option-icon" :option="option" />
            <span class="flex min-w-0 flex-1 items-center gap-2">
              <slot name="option-label" :option="option">
                <span class="truncate flex-1 text-left" :title="option.label">{{
                  option.label
                }}</span>
              </slot>
...
const displayLabel = computed(() => selectedOption.value?.label ?? selected.value);
