<template>
  <div class="space-y-1.5">
    <div v-if="emails.length" class="space-y-1.5">
      <div v-for="(item, i) in emails" :key="item.uid ?? i" class="flex flex-col gap-0.5">
        <div class="flex items-center gap-2">
          <span class="text-xs font-medium text-foreground truncate flex-1">{{
            item.subject || t("chat.tools.detail.noSubject")
          }}</span>
          <span v-if="item.received_at" class="text-[10px] text-muted-foreground shrink-0">{{
            item.received_at
          }}</span>
        </div>
        <span v-if="item.from" class="text-[10px] text-muted-foreground truncate">{{
          item.from
        }}</span>
      </div>
    </div>
