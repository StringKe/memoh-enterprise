...
export const useChatStore = defineStore("chat", () => {
  ...
  const streaming = computed(
    () => streamingSessionId.value !== null && streamingSessionId.value === sessionId.value,
  );
  ...
  function mergeBackgroundTask(
    ...
    const merged: BackgroundTask = existing
      ? { ...existing }
      : { taskId: incoming.taskId, status: incoming.status };
    ...
    merged.status =
      normalizeBackgroundStatus(merged.status, merged.event) || merged.status || "running";
  ...
  async function refreshCurrentSession(targetBotId?: string, targetSessionId?: string) {
    ...
    refreshPromise = (async () => {
      ...
      const streamStillActive =
        streamingSessionId.value === sid && pendingAssistantStream && !pendingAssistantStream.done;
  ...
  function scheduleRefreshCurrentSession(expectedSessionId?: string, delay = 100) {
    ...
    refreshTimer = setTimeout(() => {
      ...
      const streamActive =
        streamingSessionId.value === sidNow &&
        pendingAssistantStream &&
        !pendingAssistantStream.done;
  ...
  function applyBackgroundTaskEvent(event: MessageStreamEvent) {
    const incoming = normalizeBackgroundTask(
      event.task ?? (event as UIBackgroundTask),
      event.event,
    );
    if (!incoming) return;
    ...
    try {
      ...
      if (!visible.length) {
        ...
        const activeSessionId =
          sessionId.value && visible.some((session) => session.id === sessionId.value)
            ? sessionId.value
            : (visible.find((s) => s.type === "chat" || s.type === "discuss")?.id ??
              visible[0]!.id);
