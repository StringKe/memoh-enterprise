...
export default defineConfig(({ command }) => {
  ...
  if (command !== "build") {
    try {
      const { loadConfig, getBaseUrl } = require("@stringke/config") as {
        loadConfig: (path: string) => {
          "web-ui"?: { port?: number; host?: string };
        };
      ...
      port = config["web-ui"]?.port ?? defaultPort;
      host = config["web-ui"]?.host ?? defaultHost;
