...
仓库里提供了一套 kustomize 示例：`deploy/kubernetes/base`。它是可改造的起点，不是托管式 Helm chart。
...
- `memoh-runtime-installer`：DaemonSet，把 workspace executor runtime 文件复制到每个节点的 `/opt/memoh/runtime`
...
部署前先编辑 `deploy/kubernetes/base/config-secret.yaml`：
