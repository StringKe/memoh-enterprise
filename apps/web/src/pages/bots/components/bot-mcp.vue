<template>
  <MasterDetailSidebarLayout>
    <template #sidebar-header>
      <InputGroup class="shadow-none">
        ...
        <InputGroupAddon align="inline-end">
          <InputGroupButton type="button" size="icon-xs" aria-label="Search">
            <Search />
          </InputGroupButton>
        </InputGroupAddon>
      </InputGroup>
    ...
    <template #sidebar-content>
      ...
      <SidebarMenu v-for="item in filteredItems" v-else :key="item.id || '_draft'">
        <SidebarMenuItem>
          <SidebarMenuButton as-child class="justify-start py-5! px-4">
            <Toggle
              :class="[
                'py-4 border w-full text-left',
                selectedItem?.id === item.id ? 'border-border' : 'border-transparent',
              ]"
              :model-value="selectedItem?.id === item.id"
              @update:model-value="
                (v) => {
                  if (v) selectItem(item);
                }
              "
            >
              <div class="flex items-center gap-2 w-full min-w-0">
                <span class="size-2 rounded-full shrink-0" :class="statusDotClass(item)" />
                <span class="truncate flex-1">
                  {{ item.name }}
                  <span v-if="!item.id" class="text-muted-foreground text-xs">*</span>
                </span>
                <Badge v-if="item.id" variant="outline" class="shrink-0 text-[10px]">
                  {{ item.type }}
                </Badge>
                <Badge v-else variant="secondary" class="shrink-0 text-[10px]">
                  {{ $t("mcp.draft") }}
                </Badge>
              </div>
            </Toggle>
    ...
    <template #sidebar-footer>
      <div class="flex gap-2 p-2">
        <Button class="flex-1" size="sm" @click="openCreateDialog">
          <Plus class="mr-1.5" />
          {{ $t("common.add") }}
        </Button>
    ...
    <template #detail>
      <ScrollArea v-if="selectedItem" class="max-h-full h-full">
        <SettingsShell width="wide" class="space-y-6">
          ...
          <form class="grid gap-4 md:grid-cols-2" @submit.prevent="handleSubmit">
            <div class="space-y-1.5">
              ...
              <Input v-model="formData.name" :placeholder="$t('common.namePlaceholder')" />
            </div>
            ...
            <template v-if="connectionType === 'stdio'">
              <div class="space-y-1.5 md:col-span-2">
                ...
                <Input v-model="formData.command" :placeholder="$t('mcp.commandPlaceholder')" />
              </div>
              <div class="space-y-1.5 md:col-span-2">
                <Label>{{ $t("mcp.arguments") }}</Label>
                <TagsInput v-model="argsTags" :add-on-blur="true" :duplicate="true">
                  <TagsInputItem v-for="tag in argsTags" :key="tag" :value="tag">
                    <TagsInputItemText />
                    <TagsInputItemDelete />
                  </TagsInputItem>
                  <TagsInputInput
              ...
              <div class="space-y-1.5 md:col-span-2">
                ...
                <Input v-model="formData.cwd" :placeholder="$t('mcp.cwdPlaceholder')" />
              </div>
            </template>
            ...
            <template v-else>
              <div class="space-y-1.5 md:col-span-2">
                <Label>URL</Label>
                <Input v-model="formData.url" placeholder="https://example.com/mcp" />
              </div>
              <div class="space-y-1.5 md:col-span-2">
                <Label>Headers</Label>
                <KeyValueEditor
          ...
          <!-- Probe status & tools -->
          <div v-if="selectedItem.id" class="border-t pt-4 space-y-4">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
              ...
              >
                <Spinner v-if="probing" class="mr-1.5" />
                <RefreshCw v-else class="mr-1.5" />
            ...
            <div
              v-if="
                selectedItem.type !== 'stdio' &&
                (probeAuthRequired || selectedItem.auth_type === 'oauth' || oauthStatus)
              "
            ...
            >
              ...
              <div v-if="oauthStatus?.auth_server" class="text-xs text-muted-foreground">
              ...
              >
                ...
                <div v-if="oauthCallbackUrl" class="space-y-1">
                  <Label class="text-xs">
                    {{ $t("mcp.oauth.callbackUrl") }}
                  </Label>
                  <div class="flex items-center gap-1.5">
                    <code class="text-xs bg-muted px-2 py-1 rounded flex-1 break-all select-all">{{
                      oauthCallbackUrl
                    }}</code>
                    <Button
                      variant="ghost"
              ...
              <div class="flex gap-2">
                <Button
                  ...
                  :disabled="
                    oauthDiscovering ||
                    oauthAuthorizing ||
                    (oauthNeedsClientId && !oauthClientId.trim())
                  "
                  @click="handleOAuthFlow"
                >
                  <Spinner v-if="oauthDiscovering || oauthAuthorizing" class="mr-1.5" />
                  <KeyRound v-else class="mr-1.5" />
                  {{
                    oauthDiscovering
                      ? $t("mcp.oauth.discovering")
                      : oauthAuthorizing
            ...
            <div v-if="displayTools.length > 0" class="space-y-1">
              ...
              >
                <Wrench class="mt-1 text-muted-foreground shrink-0 text-xs" />
                <div class="min-w-0">
                  <span class="font-mono text-xs font-medium">{{ tool.name }}</span>
                  <p v-if="tool.description" class="text-xs text-muted-foreground truncate">
                    {{ tool.description }}
                  </p>
                </div>
              </div>
            ...
            <p v-if="selectedItem.last_probed_at" class="text-xs text-muted-foreground">
      ...
      <Empty v-else class="h-full flex justify-center items-center">
        <EmptyHeader>
  ...
  <Dialog v-model:open="createDialogOpen">
    <DialogContent class="sm:max-w-md">
      ...
      <form class="mt-4 flex flex-col gap-4" @submit.prevent="handleCreateDraft">
        <div class="space-y-1.5">
          <Label>{{ $t("common.name") }}</Label>
          <Input v-model="createName" :placeholder="$t('common.namePlaceholder')" />
        </div>
        <div class="space-y-1.5">
          <Label>{{ $t("common.type") }}</Label>
  ...
  <Dialog v-model:open="importDialogOpen">
    <DialogContent class="sm:max-w-lg w-[calc(100vw-2rem)] max-w-[calc(100vw-2rem)] sm:w-auto">
      ...
      <div class="h-87.5 rounded-md border overflow-hidden mt-3">
        <MonacoEditor v-model="importJson" language="json" />
      </div>
      <DialogFooter class="mt-4">
        ...
        <Button :disabled="importSubmitting || !importJson.trim()" @click="handleImportFromDialog">
          <Spinner v-if="importSubmitting" class="mr-1.5" />
          {{ $t("common.import") }}
        </Button>
      </DialogFooter>
    </DialogContent>
  ...
  <Dialog v-model:open="exportDialogOpen">
    <DialogContent>
      ...
      <div class="h-87.5 rounded-md border overflow-hidden mt-4">
        <MonacoEditor :model-value="exportJson" language="json" :readonly="true" />
      </div>
      <DialogFooter class="mt-4">
        <Button variant="outline" @click="handleCopyExport">
          {{ $t("common.copy") }}
        </Button>
        <DialogClose as-child>
          <Button>
...
async function handleSubmit() {
  ...
  try {
    ...
    if (isDraft.value) {
      ...
      const created =
        items.value.find((i) => i.id === savedId) ?? items.value.find((i) => i.name === body.name);
