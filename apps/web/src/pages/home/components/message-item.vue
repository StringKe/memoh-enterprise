<template>
  ...
  >
    ...
    >
      ...
      <ChannelBadge v-if="message.platform" :platform="message.platform" />
    </div>

    <!-- Content -->
    <div class="min-w-0" :class="contentClass" data-chat-content>
      ...
      <div v-else-if="message.role === 'user' && sessionType === 'schedule'" class="space-y-2">
        <ScheduleTriggerBlock v-if="message.text" :content="message.text" :bot-id="botId" />
        <AttachmentBlock
          v-if="userAttachmentBlock"
          :block="userAttachmentBlock"
          :on-open-media="onOpenMedia"
      ...
      <div v-else-if="message.role === 'user' && sessionType === 'subagent'" class="space-y-2">
        <div
          v-if="message.text"
      ...
      <div v-else-if="message.role === 'user'" class="space-y-2">
        ...
        <AttachmentBlock
          ...
          :on-open-media="onOpenMedia"
        />
        <p class="text-xs text-muted-foreground/80 mt-1 text-right" :title="fullTimestamp">
          {{ relativeTimestamp }}
        </p>
      </div>
      ...
      <div v-else class="space-y-3">
        ...
        <template v-for="(block, i) in message.messages" :key="i">
          <!-- Thinking block -->
          <ThinkingBlock
          ...
          <!-- Tool call block -->
          <ToolCallBlock v-else-if="block.type === 'tool'" :block="block as ToolCallBlockType" />
...
import { Avatar, AvatarImage, AvatarFallback } from "@stringke/ui";
...
const isSelf = computed(() => props.message.role !== "user" || props.message.isSelf !== false);
...
const isSpecialUserMessage = computed(
  () =>
    ...
    (props.sessionType === "heartbeat" ||
      props.sessionType === "schedule" ||
      props.sessionType === "subagent"),
);
...
function isAssistantBlockStreaming(index: number): boolean {
  return (
    ...
    !hasLaterAssistantMessage(index)
  );
}

const hasVisibleAssistantBlocks = computed(
  () => props.message.role === "assistant" && props.message.messages.length > 0,
);

const relativeTimestamp = computed(() => formatRelativeTime(props.message.timestamp));
const fullTimestamp = computed(() => formatDateTime(props.message.timestamp));
</script>
