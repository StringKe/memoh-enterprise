<template>
  <div class="space-y-1.5">
    ...
    >
      <span v-for="item in backgroundMeta" :key="item" class="font-mono">{{ item }}</span>
    </div>
    <pre
      v-if="progressText"
...
const backgroundOutput = computed(
  () => backgroundTask.value?.outputTail || backgroundTask.value?.chunk || "",
);
...
const backgroundMeta = computed(() => {
  ...
  if (typeof task.exitCode === "number" && !isBackgroundActive.value)
    items.push(`exit ${task.exitCode}`);
