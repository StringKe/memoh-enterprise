<template>
  <main class="w-screen h-screen flex *:m-auto bg-background relative p-4">
    <header class="absolute top-6 right-6 flex items-center gap-2">
      <Select :model-value="language" @update:model-value="(v) => v && setLanguage(v as Locale)">
        <SelectTrigger class="w-28 h-9" :aria-label="$t('settings.language')">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
          <SelectGroup>
    ...
    <section class="w-full max-w-sm flex flex-col gap-10">
      <section>
        <h1 class="scroll-m-20 text-3xl tracking-wide font-semibold text-foreground text-center">
          {{ $t("auth.welcome") }}
        </h1>
      </section>
      <form @submit="login">
