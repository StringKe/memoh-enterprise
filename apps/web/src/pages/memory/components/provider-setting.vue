<template>
  <SettingsShell v-if="curProvider" width="standard" class="space-y-6">
    ...
    <div class="space-y-2">
      ...
      <Input v-model="form.name" :placeholder="$t('memory.namePlaceholder')" />
    </div>
    ...
    <template v-if="curProvider.provider === 'builtin'">
      ...
      <div v-if="builtinCollections.length > 0" class="grid gap-3 md:grid-cols-2">
        <div
        ...
        >
          <div class="flex items-center justify-between gap-3">
            ...
            >
              {{
                collection.qdrant?.ok
                  ? $t("memory.collectionHealthy")
                  : $t("memory.collectionUnavailable")
              }}
            </span>
          </div>
    ...
    >
      ...
      >
        <Label>
          ...
          <span v-if="fieldSchema.required" class="text-destructive">*</span>
        </Label>
        <p v-if="fieldSchema.description" class="text-xs text-muted-foreground">
    ...
    <div class="flex justify-end">
      <Button :disabled="saveLoading" @click="handleSave">
        <Spinner v-if="saveLoading" class="mr-1.5" />
        {{ $t("common.save") }}
      </Button>
    </div>
  </SettingsShell>
...
function isWideField(
  key: string | number,
  schema: { secret?: boolean; type?: string; description?: string },
) {
  const keyStr = String(key).toLowerCase();
  if (schema.secret) return true;
  if (
    keyStr.includes("url") ||
    keyStr.includes("endpoint") ||
    keyStr.includes("key") ||
    keyStr.includes("token") ||
    keyStr.includes("path") ||
    keyStr.includes("uri")
  )
    return true;
