...
<template>
  <MasterDetailSidebarLayout>
    <template #sidebar-content>
      <SidebarMenu v-for="item in curFilterProvider" :key="item.id">
        <SidebarMenuItem>
          <SidebarMenuButton as-child class="justify-start py-5! px-4">
            <Toggle
              ...
              "
            >
              <Brain class="mr-2 size-4 text-primary" />
              {{ item.name }}
            </Toggle>
          </SidebarMenuButton>
        </SidebarMenuItem>
    ...
    <template #detail>
      ...
      <Empty v-else class="h-full flex justify-center items-center">
        ...
        <EmptyContent>
          <Button variant="outline" class="w-full" @click="openStatus.addOpen = true">
            <Plus class="mr-2" />
            {{ $t("memory.add") }}
          </Button>
        </EmptyContent>
