<template>
  ...
  >
    <div class="relative shrink-0 mr-2.5">
      <Avatar v-if="isIMSession" class="size-[26px] border border-border bg-accent">
        <AvatarImage v-if="avatarUrl" :src="avatarUrl" :alt="displayLabel" />
      ...
      >
        <component :is="iconComponent" class="size-2.5" :class="iconClass" />
      </div>
      <div
      ...
      >
        <ChannelBadge :platform="session.channel_type" class="size-2" />
      </div>
    </div>
    ...
    <div class="flex flex-col flex-1 min-w-0">
      <div class="flex items-center">
        ...
        <span v-if="session.updated_at" class="text-[8px] text-muted-foreground ml-1 shrink-0">
...
const displayLabel = computed(() => {
  const meta = routeMeta();
  return (
    ((meta.conversation_name as string) ?? "").trim() ||
