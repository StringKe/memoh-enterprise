<template>
  <div class="space-y-1.5">
    <div v-if="results.length" class="space-y-1.5">
      <div v-for="(item, i) in results" :key="i" class="flex flex-col gap-0.5">
        <a
          :href="item.url"
          target="_blank"
        ...
        <span v-if="item.url" class="text-[10px] text-muted-foreground truncate" :title="item.url">
          {{ item.url }}
        </span>
        <span v-if="item.description" class="text-[10px] text-muted-foreground/80 line-clamp-2">
          {{ item.description }}
        </span>
      </div>
    </div>
