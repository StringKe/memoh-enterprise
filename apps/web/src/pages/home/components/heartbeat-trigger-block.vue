<template>
  ...
  >
    ...
    <div class="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs">
      <span v-if="parsed.time" class="text-muted-foreground">{{ t("chat.heartbeatTime") }}</span>
      <span v-if="parsed.time">{{ parsed.time }}</span>
      <span v-if="parsed.interval" class="text-muted-foreground">{{
        t("chat.heartbeatInterval")
      }}</span>
      <span v-if="parsed.interval">{{ parsed.interval }}</span>
      <span v-if="parsed.lastHeartbeat" class="text-muted-foreground">{{
        t("chat.heartbeatLastAt")
      }}</span>
