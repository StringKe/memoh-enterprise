<template>
  <SettingsShell width="standard">
    ...
    <form @submit="handleSave">
          ...
          <Label
            :for="
              field.type === 'bool' || field.type === 'enum'
                ? undefined
                : `email-field-${field.key}`
            "
          >
            {{
              $te(`email.fields.${field.key}`)
                ? $t(`email.fields.${field.key}`)
      ...
      <section v-if="isOAuthProvider" class="mt-6 p-4 border rounded-lg bg-muted/30">
          ...
          <div class="flex-1 min-w-55">
            ...
            >
              ...
              <template v-else-if="oauthStatus && oauthStatus.has_token">
                {{
                  oauthStatus.email_address
                    ? $t("email.oauth.status.authorized", { email: oauthStatus.email_address })
                    : $t("email.oauth.status.authorizedUnknown")
                }}
              </template>
              <template v-else>
      ...
      <section class="flex justify-end mt-6 gap-4">
        ...
        <LoadingButton type="submit" :loading="editLoading">
...
const oauthTokenExpired = computed(() =>
