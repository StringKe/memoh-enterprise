...
function scopeIdsForTemplate(innerContent: string, ids: Set<string>): string {
  ...
  for (const id of ids) {
    ...
    out = out.replace(new RegExp(`(?<=[\\s<])id="${idRe}"`, "g"), `:id="\`\${uid}-${id}\`"`);
    ...
    out = out.replace(
      new RegExp(`(?<=[\\s<])([a-zA-Z][a-zA-Z0-9:-]*)="([^"]*url\\(#${idRe}\\)[^"]*)"`, "g"),
      (_match, attr: string, value: string) => {
        const bound = value.replace(new RegExp(`url\\(#${idRe}\\)`, "g"), `url(#\${uid}-${id})`);
        return `:${attr}="\`${bound}\`"`;
      },
    );
...
console.log("🎨 @stringke/icon generator\n");
...
const knownComponents = new Set(
  exports
    .map((line) => line.match(/from '\.\/icons\/([^']+)\.vue'/)?.[1])
