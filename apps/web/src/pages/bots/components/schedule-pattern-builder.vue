<template>
  <div class="space-y-4">
    <div class="space-y-2">
      ...
      <NativeSelect v-model="modeModel" class="h-9 text-xs">
    ...
    <!-- minutes -->
    <div v-if="state.mode === 'minutes'" class="space-y-2">
    ...
    <!-- hourly -->
    <div v-else-if="state.mode === 'hourly'" class="space-y-2">
    ...
    <!-- daily -->
    <div v-else-if="state.mode === 'daily'" class="space-y-3">
      <div class="space-y-2">
    ...
    <!-- weekly -->
    <div v-else-if="state.mode === 'weekly'" class="space-y-3">
      <div class="space-y-2">
    ...
    <!-- monthly -->
    <div v-else-if="state.mode === 'monthly'" class="space-y-3">
      <div class="space-y-2">
    ...
    <!-- yearly -->
    <div v-else-if="state.mode === 'yearly'" class="space-y-3">
      <div class="grid grid-cols-2 gap-2">
        <div class="space-y-2">
          <Label>{{ $t("bots.schedule.form.month") }}</Label>
          <NativeSelect v-model="yearlyMonthModel" class="h-9 text-xs">
            <option v-for="(key, idx) in MONTH_KEYS" :key="key" :value="String(idx + 1)">
    ...
    <!-- advanced -->
    <div v-else-if="state.mode === 'advanced'" class="space-y-2">
    ...
    <div class="rounded-md border bg-muted/30 px-3 py-2 space-y-1.5">
      <div class="flex items-start justify-between gap-2">
        <div class="min-w-0 flex-1 space-y-1">
          ...
          <code v-if="previewPattern" class="text-xs font-mono break-all">{{
            previewPattern
          }}</code>
          <code v-else class="text-xs text-muted-foreground">—</code>
        </div>
      </div>
      <div class="text-xs">
        <span v-if="humanText" class="text-foreground">{{ humanText }}</span>
        <span v-else class="text-destructive">{{ $t("bots.schedule.form.invalidPattern") }}</span>
      </div>
      <div
      ...
      >
        ...
        <div v-for="(d, i) in upcomingRuns" :key="i" class="font-mono">
          · {{ formatPreviewDate(d) }}
        </div>
      </div>
    </div>
...
import { Input, Label, NativeSelect } from "@stringke/ui";
import {
  describeCron,
  nextRuns,
  toCron,
...
const previewFormatter = computed(
  () =>
    new Intl.DateTimeFormat(locale.value.startsWith("zh") ? "zh-CN" : "en-US", {
