<template>
  <div class="p-6 max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex flex-wrap items-end gap-4">
      ...
      <div v-if="modelOptions.length > 0" class="space-y-1.5">
        ...
        <Select v-model="selectedModelId">
          ...
          <SelectContent>
            ...
            <SelectItem v-for="m in modelOptions" :key="m.model_id" :value="m.model_id!">
    ...
    <template v-else>
      ...
      <div v-if="hasData" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card v-if="byModelData.length > 0">
          <CardHeader class="pb-2 flex flex-row items-center justify-between">
            ...
            <Select v-model="modelChartType" class="w-auto">
        ...
        <Card>
          ...
          <CardContent>
            <VChart style="height: 300px; width: 100%" :option="dailyTokensOption" autoresize />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <VChart style="height: 300px; width: 100%" :option="cacheBreakdownOption" autoresize />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <VChart style="height: 300px; width: 100%" :option="cacheHitRateOption" autoresize />
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader class="pb-2 flex flex-row items-center justify-between">
          ...
          <span v-if="recordsPaginationSummary" class="text-xs text-muted-foreground tabular-nums">
        ...
        <CardContent class="space-y-3">
          ...
          <div v-if="recordsTotalPages > 1" class="flex justify-end">
            <Pagination
              :total="recordsTotal"
...
import { getBotsByBotIdTokenUsage, getBotsByBotIdTokenUsageRecords } from "@stringke/sdk";
import BotSelect from "@/components/bot-select/index.vue";
import type {
  HandlersDailyTokenUsage,
  HandlersModelTokenUsage,
  HandlersTokenUsageRecord,
} from "@stringke/sdk";
...
const modelOptions = computed(() => byModelData.value.filter((m) => m.model_id));
...
const hasData = computed(() => {
  ...
  return (
    chat.length > 0 || heartbeat.length > 0 || schedule.length > 0 || byModelData.value.length > 0
  );
});
...
const cacheBreakdownOption = computed(() => {
  ...
  function sumField(
    day: string,
    field: "cache_read_tokens" | "cache_write_tokens" | "input_tokens",
  ) {
    let total = 0;
    for (const tp of types) {
