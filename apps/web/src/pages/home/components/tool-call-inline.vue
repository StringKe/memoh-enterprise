<template>
  <div class="text-sm leading-relaxed">
    ...
    >
      <component :is="display.icon" class="size-3.5 shrink-0" />
      <span v-if="!display.hideAction" class="shrink-0" :class="actionClass">{{
        actionLabel
      }}</span>
      <button
      ...
      <span
        ...
        >{{ display.target }}</span
      >
      <span v-if="display.diffAdd" class="font-mono shrink-0 text-emerald-600 dark:text-emerald-500"
        >+{{ display.diffAdd }}</span
      >
      <span v-if="display.diffRemove" class="font-mono shrink-0 text-rose-600 dark:text-rose-500"
        >-{{ display.diffRemove }}</span
      >
      <span v-if="display.errorSuffix" class="font-mono shrink-0">{{ display.errorSuffix }}</span>
      <span
        v-if="approvalLabel"
      ...
      />
      <ChevronDown v-else class="size-3.5 shrink-0 ml-auto opacity-60 group-hover:opacity-100" />
    </div>

    <div v-else class="flex items-center gap-1.5 w-full py-0.5" :class="rowClass">
      <component :is="display.icon" class="size-3.5 shrink-0" />
      <span v-if="!display.hideAction" class="shrink-0" :class="actionClass">{{
        actionLabel
      }}</span>
    ...
    <div v-if="expandable && open" class="mt-1 ml-5 py-1 space-y-1.5">
      <component :is="display.detail" v-if="display.detail" :block="block" />
      <ToolCallDetailGeneric v-else :block="block" />
    </div>

    <div v-if="canRespondApproval" class="mt-1.5 ml-5 flex items-center gap-2">
      <Button
        size="sm"
...
import { getToolDisplay, isDirPathTool, isFilePathTool } from "./tool-call-registry";
...
const approvalLabel = computed(() => {
  ...
  if (approval.status === "pending")
    return `${id} ${t("chat.tools.pendingApproval", "pending approval")}`.trim();
...
const canRespondApproval = computed(() => {
  const approval = props.block.approval;
  return Boolean(
    approval?.approval_id && approval.status === "pending" && approval.can_approve !== false,
  );
});
...
const canOpenInFiles = computed(() => Boolean(filePath.value) && Boolean(openInFileManager));
