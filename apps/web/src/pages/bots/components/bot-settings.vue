...
const searchProviders = computed(() =>
  (searchProviderData.value ?? []).filter((p) => p.enable !== false),
);
const memoryProviders = computed(() => memoryProviderData.value ?? []);
const ttsProviders = computed(() =>
  (ttsProviderData.value ?? []).filter((p) => p.enable !== false),
);
const enabledTtsProviderIds = computed(() => new Set(ttsProviders.value.map((p) => p.id)));
const transcriptionProviders = computed(() =>
  (transcriptionProviderData.value ?? []).filter(
    (p: Record<string, unknown>) => p.enable !== false,
  ),
);
const enabledTranscriptionProviderIds = computed(
  () => new Set(transcriptionProviders.value.map((p: Record<string, unknown>) => p.id as string)),
);
const ttsModels = computed(() =>
  (ttsModelData.value ?? []).filter((m: Record<string, unknown>) =>
    enabledTtsProviderIds.value.has(m.provider_id as string),
  ),
);
const transcriptionModels = computed(() =>
  (transcriptionModelData.value ?? []).filter((m: Record<string, unknown>) =>
    enabledTranscriptionProviderIds.value.has(m.provider_id as string),
  ),
);
...
const selectedMemoryProviderType = computed(() => selectedMemoryProvider.value?.provider ?? "");
...
const selectedBuiltinMemoryMode = computed(
  () =>
    (selectedBuiltinMemoryProvider.value?.config as Record<string, string> | undefined)
      ?.memory_mode || "off",
);
...
const availableReasoningEfforts = computed(() => {
  ...
  const efforts = (
    (model?.config as { reasoning_efforts?: string[] } | undefined)?.reasoning_efforts ?? []
