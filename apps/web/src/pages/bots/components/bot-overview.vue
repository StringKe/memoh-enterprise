...
const checksSummaryText = computed(() => {
  const issueCount = checks.value.filter(
  ...
  <div>
    <div class="rounded-md border p-4">
      <div class="flex items-center justify-between gap-2">
        ...
        <Button variant="outline" size="sm" :disabled="checksLoading" @click="handleRefreshChecks">
          <Spinner v-if="checksLoading" class="mr-1.5" />
          {{ $t("common.refresh") }}
        </Button>
      </div>
      <div class="mt-3 flex items-center gap-2 text-xs">
        <Badge :variant="hasIssue ? 'destructive' : 'default'" class="text-xs">
          {{ checksSummaryText }}
        </Badge>
      </div>
      ...
      <p v-else-if="checks.length === 0" class="mt-4 text-xs text-muted-foreground">
        {{ $t("bots.checks.empty") }}
      </p>

      <ul v-else class="mt-4 divide-y">
        <li v-for="item in checks" :key="item.id" class="py-3 first:pt-0 last:pb-0">
          <div class="flex items-center justify-between gap-2">
            <div class="min-w-0">
              <p class="font-mono text-xs">
                {{ checkTitleLabel(item) }}
              </p>
              <p v-if="item.subtitle" class="mt-0.5 text-xs text-muted-foreground">
                {{ item.subtitle }}
              </p>
            </div>
            <Badge :variant="checkStatusVariant(item.status)" class="text-[10px]">
          ...
          <p v-if="item.detail" class="mt-1 text-xs text-muted-foreground break-all">
            {{ item.detail }}
          </p>
        </li>
      </ul>
