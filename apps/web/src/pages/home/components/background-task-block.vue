<template>
  <div
    class="flex items-start gap-2 py-1.5 text-xs text-muted-foreground border-y border-border/60"
  >
    <component :is="statusIcon" class="size-3.5 shrink-0 mt-0.5" :class="iconClass" />
    <div class="min-w-0 flex-1 space-y-0.5">
      <div class="flex items-center gap-1.5 min-w-0">
        <span class="shrink-0" :class="labelClass">{{ statusLabel }}</span>
        <span
          v-if="task.command"
      ...
      <div v-if="metaText" class="font-mono truncate text-muted-foreground/80" :title="metaText">
        {{ metaText }}
      </div>
    </div>
  </div>
