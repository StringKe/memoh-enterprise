  ...
  <Dialog :open="open" @update:open="$emit('update:open', $event)">
    <DialogContent class="sm:max-w-lg">
      ...
      <div class="space-y-4 py-2">
        ...
        <div v-if="mcp" class="rounded-md border border-border p-3 space-y-1">
          <div class="flex items-center gap-2">
            <p class="text-xs font-medium">
              {{ mcp.name }}
            </p>
            <Badge v-if="mcp.transport" variant="outline" size="sm">
              {{ mcp.transport }}
            </Badge>
          </div>
      ...
      <DialogFooter>
        ...
        <Button :disabled="!selectedBotId" @click="handleNavigate">
