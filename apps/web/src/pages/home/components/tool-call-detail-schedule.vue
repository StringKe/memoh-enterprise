<template>
  <div class="space-y-1.5">
    <div v-if="items.length" class="space-y-1">
      <div v-for="(item, i) in items" :key="item.id ?? i" class="flex flex-col gap-0.5 text-xs">
        <div class="flex items-center gap-2">
          <span class="text-foreground truncate flex-1">{{
            item.name || item.id || t("chat.tools.detail.unnamedSchedule")
          }}</span>
          <span
            v-if="item.pattern"
        ...
        <span v-if="item.prompt" class="text-[10px] text-muted-foreground line-clamp-2">{{
          item.prompt
        }}</span>
      </div>
    </div>
