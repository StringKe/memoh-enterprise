<template>
  <SidebarProvider class="min-h-[initial]! absolute inset-0" :default-open="true">
    <Sidebar class="relative! **:[[role=navigation]]:relative! sidebar-container h-full!">
    ...
    <SidebarInset class="min-w-0 overflow-hidden">
      ...
      <div class="fixed right-4 top-0 h-12 z-1000 md:hidden flex items-center">
        <Menu class="cursor-pointer p-2" @click="mobileOpen = !mobileOpen" />
      </div>

      <Sheet :open="mobileOpen" @update:open="(v: boolean) => (mobileOpen = v)">
        <SheetContent
