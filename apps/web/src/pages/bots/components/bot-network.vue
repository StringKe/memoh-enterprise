...
const { mutateAsync: runNetworkAction, isLoading: isLoggingOut } = useMutation({
  mutation: (actionID: string) => executeBotNetworkAction(props.botId, actionID, {}),
  onSettled: () => {
    queryCache.invalidateQueries({ key: ["bot-network-status", props.botId] });
  },
});
...
const hasChanges = computed(() => {
  ...
  return (
    form.overlay_enabled !== (s.overlay_enabled ?? false) ||
    form.overlay_provider !== (s.overlay_provider ?? "") ||
    JSON.stringify(form.overlay_config ?? {}) !==
      JSON.stringify((s.overlay_config as Record<string, unknown> | undefined) ?? {})
  );
});
...
watch(
  ...
  (val) => {
    ...
    form.overlay_config = cloneConfig(
      (val.overlay_config as Record<string, unknown> | undefined) ?? {},
    );
    void nextTick(() => {
