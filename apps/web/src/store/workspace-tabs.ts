...
export const useWorkspaceTabsStore = defineStore("workspace-tabs", () => {
  ...
  function ensureBot(botId: string | null | undefined): BotTabState | null {
    ...
    if (!storage.value[bid]) {
      ...
      const tabs = (
        currentTabs as Array<WorkspaceTab | { id: string; type: string; title: string }>
      ).map((tab) =>
      ...
      if (
        cur.terminalCounter === undefined ||
        cur.displayCounter === undefined ||
        cur.dirtyFileTabs === undefined ||
        tabsChanged
      ) {
        storage.value = {
          ...storage.value,
          [bid]: {
            ...
            displayCounter:
              cur.displayCounter ?? (tabs.some((tab) => tab.type === "display") ? 1 : 0),
  ...
  function updateChatTitle(sessionId: string, title: string) {
    ...
    const next = state.tabs.map((t) => (t.id === id && t.type === "chat" ? { ...t, title } : t));
