<template>
  <div class="flex-1 flex flex-col h-full min-w-0 relative">
    <div v-if="!currentBotId" class="flex-1 flex items-center justify-center">
    ...
    <template v-else>
          ...
          <ScrollArea ref="scrollContainer" class="h-full">
            <div class="w-full max-w-4xl mx-auto px-10 pt-6 pb-6 space-y-6">
              ...
              >
                ...
                <p
                  v-else-if="
                    activeSession?.type === 'heartbeat' || activeSession?.type === 'schedule'
                  "
      ...
      <div v-if="!activeChatReadOnly" class="px-3 sm:px-5 lg:px-8 py-2.5">
          ...
          <section>
            <InputGroup class="bg-transparent overflow-hidden shadow-none! ring-0! border-border!">
              <InputGroupTextarea
                ...
                :placeholder="
                  activeChatReadOnly ? $t('chat.readonlyHint') : $t('chat.inputPlaceholder')
                "
                ...
                @paste="handlePaste"
              />
              <InputGroupAddon align="block-end" class="items-center py-1.5">
                ...
                <Popover v-model:open="reasoningPopoverOpen">
                  <PopoverTrigger as-child>
                    <Button
                      ...
                      variant="ghost"
                      :disabled="
                        !currentBotId || activeChatReadOnly || !activeModelSupportsReasoning
                      "
                ...
                <SessionInfoRing class="ml-auto" :override-model-id="overrideModelId" />
                ...
                <Button
                  ...
                  size="icon"
                  :disabled="
                    (!inputText.trim() && !pendingFiles.length) ||
                    !currentBotId ||
                    activeChatReadOnly
                  "
...
// child-index DOM shape of @stringke/ui's ScrollArea (which wraps reka-ui).
...
const { y, directions, arrivedState } = useScroll(scrollEl, {
...
function isSentinelStillInRange(scrollElement: HTMLElement): boolean {
  ...
  return sentinelRect.bottom >= rootRect.top - 200 && sentinelRect.top <= rootRect.bottom;
}
