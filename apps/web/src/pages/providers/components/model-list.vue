<template>
  <section>
    <section class="flex justify-between items-center mb-4">
      ...
      <div v-if="providerId" class="flex items-center gap-2 ml-auto">
    ...
    <template v-if="models && models.length > 0">
      <InputGroup v-if="models.length > 5" class="shadow-none mb-4">
        <InputGroupAddon align="inline-start">
          <Search class="size-3.5 text-muted-foreground" />
        </InputGroupAddon>
        <InputGroupInput v-model="searchQuery" :placeholder="$t('models.searchModelPlaceholder')" />
      </InputGroup>
      ...
      <Empty v-if="filteredModels.length === 0" class="flex justify-center items-center py-8">
        <EmptyHeader>
...
const pageEnd = computed(() =>
  Math.min(currentPage.value * PAGE_SIZE, filteredModels.value.length),
);
