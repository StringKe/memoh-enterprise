...
Memoh 支持 **斜杠命令**，在进 LLM 之前截获。用来快速看状态、改配置、切模型、开会话、停生成等。外接渠道和 API 本地通道都支持；**解析命令本身一般不吃模型 token**（与真正进对话的内容无关）。
...
默认：API 本地通道多 `chat`；私聊多 `chat`；外接群多 `discuss`。

**API 本地通道** 没有 `/new discuss`，要 discuss 请用 Telegram、Discord 等。
