<template>
  <div class="space-y-4">
    <div v-for="schemaField in visibleFields" :key="schemaField.key" class="space-y-2">
      <FieldContent :field="schemaField" :id-prefix="idPrefix" :disabled="disabled" />
    </div>

    <div v-if="collapsedFields.length" class="space-y-4">
      <button
        type="button"
        class="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors"
        @click="showCollapsed = !showCollapsed"
      >
        <component :is="showCollapsed ? ChevronDown : ChevronRight" class="size-3.5" />
        {{ t("common.advanced") }}
      </button>

      <template v-if="showCollapsed">
        <div v-for="schemaField in collapsedFields" :key="schemaField.key" class="space-y-2">
          <FieldContent :field="schemaField" :id-prefix="idPrefix" :disabled="disabled" />
        </div>
      </template>
    </div>
          ...
          h("div", { class: "relative" }, [
            ...
            h(
              "button",
              {
                ...
                disabled: fieldProps.disabled,
                onClick: () => {
                  visibleSecrets[field.key] = !visibleSecrets[field.key];
                },
              },
