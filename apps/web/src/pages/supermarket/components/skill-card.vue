<template>
  <Card class="flex flex-col">
        ...
        <div class="flex items-center gap-2 min-w-0">
          ...
          <div class="min-w-0">
            <div class="flex items-center gap-1.5">
              <CardTitle class="text-sm truncate" :title="skill.name">
                {{ skill.name }}
              </CardTitle>
              <a
            ...
            <span v-if="skill.metadata?.author?.name" class="text-[11px] text-muted-foreground">
    ...
    <CardFooter class="pt-0 flex items-center justify-between gap-2">
      ...
      <Button size="sm" class="shrink-0" @click.stop="$emit('install', skill)">
