<template>
  ...
  >
    ...
    <div class="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs">
      <span v-if="parsed.name" class="text-muted-foreground">{{ t("chat.scheduleName") }}</span>
      <span v-if="parsed.name">{{ parsed.name }}</span>
      <span v-if="parsed.pattern" class="text-muted-foreground">{{
        t("chat.schedulePattern")
      }}</span>
      <span v-if="parsed.pattern">
        <code class="text-[11px] px-1 py-0.5 rounded bg-amber-100/50 dark:bg-amber-900/30">{{
          parsed.pattern
        }}</code>
      </span>
      <span v-if="parsed.description" class="text-muted-foreground">{{
        t("chat.scheduleDescription")
      }}</span>
      <span v-if="parsed.description">{{ parsed.description }}</span>
      <span v-if="parsed.maxCalls" class="text-muted-foreground">{{
