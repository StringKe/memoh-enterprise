...
<template>
  <div class="w-full">
    ...
    >
      <FolderOpen class="mb-2 size-8 opacity-40" />
    ...
    <div v-else>
      <!-- Header row -->
      <div
        class="flex items-center border-b border-border px-3 py-2 text-xs font-medium text-muted-foreground"
      >
        <div class="flex-1">
          {{ t("bots.files.name") }}
        </div>
      ...
      <!-- File rows -->
      <ContextMenu v-for="entry in sortedEntries" :key="entry.path">
        ...
        <ContextMenuContent>
          <ContextMenuItem v-if="!entry.isDir" @select="emit('download', entry)">
