<template>
  <div
    class="flex items-center h-12 shrink-0 border-b border-border bg-background gap-1 [-webkit-app-region:drag]"
  >
    <div
    ...
    >
      ...
      >
        <component :is="tabIcon(tab)" class="size-3.5 shrink-0" />
        <span class="truncate">
          {{ resolveTitle(tab) }}
        </span>
        <span
    ...
    <div class="flex items-center shrink-0 px-1.5 pt-2 pb-1 gap-0.5 border-l border-border">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem :disabled="!tabs.length" @select="store.closeAll()">
            {{ t("chat.tabBarToolkit.closeAll") }}
          </DropdownMenuItem>
          <DropdownMenuItem :disabled="!tabs.length" @select="store.closeFinished()">
