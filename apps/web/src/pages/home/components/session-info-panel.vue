<template>
  <ScrollArea class="h-full">
    <div class="px-4 py-3">
      <!-- No session -->
      <div v-if="!sessionId" class="flex items-center justify-center h-40">
      ...
      <template v-else>
        ...
        <div class="divide-y divide-border text-xs">
          ...
          <div class="py-2 space-y-1.5">
            <div class="flex items-center justify-between">
              ...
              <span class="font-medium text-foreground tabular-nums">
                ...
                <template v-else> {{ formatTokenCount(usedTokens) }} / -- </template>
              </span>
            </div>
            <div
          ...
          <div class="flex items-center justify-between py-2">
            ...
            <span class="font-medium text-foreground tabular-nums">{{
              formatTokenCount(info?.cache_stats?.cache_write_tokens ?? 0)
            }}</span>
          </div>
        </div>
        ...
        <div class="mt-3">
          ...
          <div v-if="!skills.length" class="text-xs text-muted-foreground">
            {{ $t("chat.infoNoSkills") }}
          </div>
...
function openSkillFile(skillName: string) {
  openInFileManager?.(
    effectiveSkillPathByName.value[skillName] || `/data/skills/${skillName}/SKILL.md`,
    false,
  );
}
