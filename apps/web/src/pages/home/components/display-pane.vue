<template>
  ...
  >
    ...
    >
      <div class="w-full max-w-[520px] rounded-lg border border-border bg-background p-5">
        ...
        <div class="mt-5 grid grid-cols-4 gap-2">
          ...
          >
            <component :is="stage.icon" class="size-4" :class="{ 'animate-pulse': stage.active }" />
    ...
    >
      ...
      >
        <span>{{
          statsVisible ? t("chat.display.stats.hide") : t("chat.display.stats.show")
        }}</span>
    ...
    >
      ...
      <div class="mb-2 rounded border border-white/15 bg-white/[0.03] px-2 py-1.5">
        ...
        <div class="grid grid-cols-[46px_minmax(0,1fr)] gap-2">
          <div
            class="flex flex-col justify-between py-1 text-right text-[10px] leading-none text-white/45"
          >
...
interface DisplayPrepareStreamEvent {
  type: string;
  step: string;
  message: string;
  percent: number;
...
const statusLabel = computed(() => {
  ...
  switch (status.value) {
    case "connecting":
      return t("chat.display.status.connecting");
    case "connected":
      return t("chat.display.status.connected");
    case "disconnected":
      return t("chat.display.status.disconnected");
    default:
      return t("chat.display.status.idle");
  }
});

const preparePercent = computed(() =>
  Math.max(0, Math.min(100, Math.round(prepareProgress.value?.percent ?? 0))),
);

const prepareStageOrder = [
  "checking",
  "system",
  "installing",
  "browser",
...
const prepareStages = computed<PrepareStage[]>(() => {
  ...
  return [
    ...
    { key: "session", label: t("chat.display.prepare.stageSession"), icon: Monitor },
  ].map((stage) => ({
    ...stage,
...
const statsRows = computed(() => [
  ...
  {
    key: "resolution",
    label: t("chat.display.stats.resolution"),
    value: displayStats.value.resolution,
  },
  ...
  {
    key: "connection",
    label: t("chat.display.stats.connection"),
    value: displayStats.value.connection,
  },
  ...
  {
    key: "loss",
    label: t("chat.display.stats.packetsLost"),
    value: displayStats.value.packetsLost,
  },
...
const bitrateChartPoints = computed(() =>
  buildBitrateChartPoints(bitrateSamples.value, bitrateChartMax.value),
);
...
const bitrateChartEndLabel = computed(() =>
  bitrateSamples.value.length ? formatChartDuration(bitrateChartWindowMs.value) : "--",
);
...
function formatUnavailableReason(reason: string): string {
  switch (reason) {
    ...
    case "display session unavailable":
      return t("chat.display.unavailable.session");
...
function closeRemoteSession() {
  ...
  if (!sessionID) return;
  void connectClients.containers
    .closeDisplaySession({
      botId: props.botId,
      sessionId: sessionID,
    })
...
function startSnapshotCapture() {
  if (snapshotTimer || snapshotFrameRequest !== null || document.visibilityState === "hidden")
    return;
...
async function updateDisplayStats() {
  ...
  try {
    ...
    const codec = inbound
      ? (report.get(String(inbound.codecId || "")) as StatsRecord | undefined)
      : undefined;
    const video = videoRef.value;
...
async function createDisplayAnswer(pc: RTCPeerConnection): Promise<DisplayOfferResponse> {
  ...
  const data = await connectClients.containers.createDisplayWebRTCOffer({
    botId: props.botId,
    type: local.type,
    sdp: local.sdp,
    sessionId: displaySessionId.value || undefined,
    candidateHost: window.location.hostname,
  });
  if (!data.sdp) {
    throw new Error("display WebRTC answer is empty");
  }
  return { type: "answer", sdp: data.sdp, sessionId: data.sessionId };
}
...
function findStats(
  ...
  predicate: (item: StatsRecord) => boolean,
...
function findSelectedCandidatePair(report: RTCStatsReport): StatsRecord | undefined {
  const transport = findStats(
    report,
    (item) => item.type === "transport" && typeof item.selectedCandidatePairId === "string",
  );
  if (transport?.selectedCandidatePairId) {
    const selected = report.get(String(transport.selectedCandidatePairId)) as
      | StatsRecord
      | undefined;
...
function appendBitrateSample(timestamp: number, kbps: number) {
  bitrateSamples.value = [...bitrateSamples.value, { timestamp, kbps: Math.max(0, kbps) }].slice(
    -BITRATE_SAMPLE_LIMIT,
  );
}
    ...
    .map((sample) => {
      const x =
        duration <= 0
          ...
          : BITRATE_CHART_PADDING +
            ((sample.timestamp - firstTimestamp) / duration) * drawableWidth;
...
function canPrepareDisplay(info: DisplayInfoPayload): boolean {
  const reason = info.unavailableReason ?? "";
  if (!info.enabled) return false;
  if (reason === "container not reachable" || reason === "manager not configured") return false;
  if (info.encoderAvailable === false && reason === "gstreamer unavailable") return false;
  return (
    !info.available ||
    !info.running ||
    info.sessionAvailable === false ||
...
function prepareEventMessage(event: DisplayPrepareStreamEvent): string {
  switch (event.step) {
    case "checking":
      return t("chat.display.prepare.checking");
    case "toolkit":
      return t("chat.display.prepare.toolkit");
    case "system":
      return t("chat.display.prepare.system");
    case "installing":
      return t("chat.display.prepare.installing");
    case "browser":
      return t("chat.display.prepare.browser");
    case "starting":
      return t("chat.display.prepare.starting");
    case "session":
      return t("chat.display.prepare.session");
    case "complete":
      return t("chat.display.prepare.complete");
    default:
      return event.message || t("chat.display.prepare.default");
  }
}
...
async function prepareDisplay(): Promise<boolean> {
  ...
  try {
    for await (const event of connectClients.containers.prepareDisplay({ botId: props.botId })) {
      if (event.type === "error") {
...
async function connect() {
  ...
  try {
    ...
    if (!info.available || !info.running) {
      status.value = "unavailable";
      unavailableReason.value = formatUnavailableReason(info.unavailableReason ?? "");
      prepareProgress.value = null;
      return;
    }
    if (info.sessionAvailable === false) {
      status.value = "unavailable";
      unavailableReason.value = formatUnavailableReason("display session unavailable");
      prepareProgress.value = null;
      return;
    }
    if (info.browserAvailable === false) {
      status.value = "unavailable";
  ...
  try {
    ...
    displaySessionId.value = answer.sessionId ?? "";
