  ...
  switch (req.action) {
    ...
    case "type": {
      ...
      await page
        ...
        .pressSequentially(req.text, { timeout: req.timeout ?? 5000 });
    ...
    case "select": {
      ...
      const selected = await page.selectOption(req.selector, req.value, {
    ...
    case "screenshot_annotate": {
      ...
      await page.evaluate(() => {
        (globalThis as any).document
          .querySelectorAll(".__memoh_annotation__")
    ...
    case "scroll": {
      ...
      if (req.selector) {
        await page.locator(req.selector).evaluate(
          (el, { dx, dy }) => {
            el.scrollBy(dx, dy);
          },
          { dx: deltaX, dy: deltaY },
