      ...
      >
        <CardHeader class="min-w-0 pb-3">
          <div class="flex min-w-0 items-center justify-between gap-2">
            <CardTitle class="min-w-0 flex-1 truncate text-sm" :title="skill.name">
              {{ skill.name }}
            </CardTitle>
            <div class="flex items-center gap-1 shrink-0">
              ...
              >
                <Spinner v-if="isSkillActionPending(skill, 'enable')" class="size-3.5" />
              ...
              >
                <Spinner v-if="isSkillActionPending(skill, 'disable')" class="size-3.5" />
              ...
              <Button
                ...
                :title="
                  skill.state === 'shadowed'
                    ? $t('bots.skills.adoptBlocked')
                    : $t('bots.skills.adoptAction')
                "
                @click="handleSkillAction('adopt', skill)"
              >
                <Spinner v-if="isSkillActionPending(skill, 'adopt')" class="size-3.5" />
        ...
        <CardContent class="mt-auto min-w-0 space-y-1.5 pt-0">
          <div class="flex flex-wrap items-center gap-1.5">
            <Badge variant="secondary" size="sm" class="rounded-full">
              {{
                skill.managed ? $t("bots.skills.managedBadge") : $t("bots.skills.discoveredBadge")
              }}
            </Badge>
            <Badge variant="outline" size="sm" class="rounded-full">
      ...
      <DialogContent class="sm:max-w-2xl max-h-[calc(100dvh-2rem)] flex flex-col overflow-hidden">
        ...
        <div class="basis-[400px] flex-1 min-h-0 py-4">
          <div class="h-full rounded-md border overflow-hidden">
            <MonacoEditor v-model="draftRaw" language="markdown" :readonly="isSaving" />
          </div>
        </div>
      ...
      <DialogContent class="sm:max-w-xl">
        ...
        <div class="space-y-4 py-2">
          <div class="space-y-2">
            ...
            <div
              class="rounded-md border bg-muted/30 px-3 py-2 font-mono text-xs text-foreground break-all"
            >
              {{ MANAGED_SKILL_PATH }}
            </div>
            <p class="text-xs text-muted-foreground">
          ...
          <div class="space-y-2">
            ...
            />
            <p v-if="discoveryRootError" class="text-xs text-destructive">
              {{ discoveryRootError }}
            </p>
          </div>
        ...
        <DialogFooter>
          ...
          <Button :disabled="!canSaveDiscoveryRoots" @click="handleSaveDiscoveryRoots">
            <Spinner v-if="isSavingDiscoveryRoots" class="mr-2 size-4" />
            {{ $t("common.save") }}
          </Button>
        </DialogFooter>
      </DialogContent>
...
const normalizedDiscoveryRootDrafts = computed(() =>
  normalizeDiscoveryRoots(parseDiscoveryRoots(discoveryRootsDraft.value)),
);
const isDiscoveryRootsDirty = computed(
  () => !areStringListsEqual(normalizedDiscoveryRootDrafts.value, savedDiscoveryRoots.value),
);
const savedDiscoveryRootsText = computed(() => savedDiscoveryRoots.value.join("\n"));
const isDiscoveryDraftModified = computed(
  () => discoveryRootsDraft.value !== savedDiscoveryRootsText.value,
);
const usesDefaultDiscoveryRoots = computed(() =>
  areStringListsEqual(savedDiscoveryRoots.value, DEFAULT_DISCOVERY_ROOTS),
);
const showDiscoveryIndicator = computed(
  () => !usesDefaultDiscoveryRoots.value || isDiscoveryRootsDirty.value,
);
const discoveryControlsDisabled = computed(() => isSavingDiscoveryRoots.value || !bot.value);
const canSaveDiscoveryRoots = computed(() => {
  return (
    !!bot.value &&
    isDiscoveryRootsDirty.value &&
    !hasDiscoveryRootErrors.value &&
    !isSavingDiscoveryRoots.value
  );
});
...
function withDiscoveryRootsMetadata(
...
function isSkillActionPending(skill: SkillItem, action: string) {
  return (
    isActioning.value && actionTargetPath.value === skill.source_path && actionName.value === action
  );
}
...
async function handleSaveDiscoveryRoots() {
  ...
  try {
    ...
    await Promise.all([refetchBot(), fetchSkills()]);
  } catch (error) {
...
watch(isDiscoveryDialogOpen, (open, prevOpen) => {
  if (
    ...
    prevOpen &&
    !isSavingDiscoveryRoots.value &&
    (isDiscoveryDraftModified.value || hasDiscoveryRootErrors.value)
  ) {
    resetDiscoveryRoots();
  }
});
