<template>
  <div class="space-y-1.5">
    <div v-if="subject" class="text-xs font-medium text-foreground">
      {{ subject }}
    </div>
    <div v-if="from" class="text-[10px] text-muted-foreground">
      {{ from }}
    </div>
    <pre
      v-if="body"
      ...
      >{{ body }}</pre
    >
    <p v-if="!subject && !from && !body" class="text-xs text-muted-foreground italic">
