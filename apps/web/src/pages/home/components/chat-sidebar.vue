<template>
  <div class="flex shrink-0 h-full relative" :style="{ width: `${sidebarWidth}px` }">
    <div class="flex flex-col h-full flex-1 min-w-0 bg-sidebar border-r border-border">
      <div
        class="flex items-center h-12 shrink-0 border-b border-border bg-sidebar/60 px-1.5 pt-1 pb-1 gap-1 overflow-x-auto overflow-y-hidden [-webkit-app-region:drag]"
      >
        <button
      ...
      <div class="flex-1 min-h-0 relative">
        <div v-show="activeTab === 'sessions'" class="absolute inset-0">
          <ChatSidebarSessions />
        </div>
        <div v-show="activeTab === 'files'" class="absolute inset-0">
          <ChatSidebarFiles v-if="currentBotId" ref="filesPanelRef" :bot-id="currentBotId" />
          <div v-else class="flex items-center justify-center h-full text-xs text-muted-foreground">
            {{ t("chat.selectBotHint") }}
          </div>
        </div>
        <div v-show="activeTab === 'skills'" class="absolute inset-0">
          <ChatSidebarSkills v-if="currentBotId" :bot-id="currentBotId" />
          <div v-else class="flex items-center justify-center h-full text-xs text-muted-foreground">
            {{ t("chat.selectBotHint") }}
          </div>
        </div>
        <div v-show="activeTab === 'mcp'" class="absolute inset-0">
          <ChatSidebarMcp v-if="currentBotId" :bot-id="currentBotId" />
          <div v-else class="flex items-center justify-center h-full text-xs text-muted-foreground">
            {{ t("chat.selectBotHint") }}
          </div>
        </div>
        <div v-show="activeTab === 'schedule'" class="absolute inset-0">
          <ChatSidebarSchedule v-if="currentBotId" :bot-id="currentBotId" />
          <div v-else class="flex items-center justify-center h-full text-xs text-muted-foreground">
            {{ t("chat.selectBotHint") }}
          </div>
        </div>
