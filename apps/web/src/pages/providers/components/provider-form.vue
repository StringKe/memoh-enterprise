  ...
  <form @submit="editProvider">
    <div class="grid gap-4 md:grid-cols-2">
        ...
        <FormItem class="md:col-span-2">
          ...
          <FormControl>
            <Input
              type="password"
              :placeholder="
                getStoredSecret(props.provider?.config as Record<string, unknown> | undefined) ||
      ...
      >
        <FormItem class="md:col-span-2">
          ...
          <FormControl>
            <Select :model-value="value || '5m'" @update:model-value="handleChange">
              <SelectTrigger>
      ...
      >
        <div class="space-y-1">
            ...
            {{
              $t(
                form.values.client_type === "github-copilot"
                  ? "provider.oauth.githubDeviceTitle"
                  : "provider.oauth.openaiTitle",
              )
            }}
          </div>
          <div class="text-muted-foreground">
            {{
              $t(
                form.values.client_type === "github-copilot"
                  ? "provider.oauth.githubDeviceDescription"
                  : "provider.oauth.openaiDescription",
              )
            }}
          </div>
          <div class="text-xs" :class="oauthExpired ? 'text-destructive' : 'text-muted-foreground'">
            ...
            <template v-else-if="oauthStatus?.has_token">
              {{
                $t(
                  form.values.client_type === "github-copilot"
                    ? "provider.oauth.status.authorizedCurrent"
                    : "provider.oauth.status.authorized",
                )
              }}
            </template>
          ...
          <div v-if="oauthStatus?.callback_url" class="text-xs text-muted-foreground">
        ...
        >
          ...
          <div class="space-y-1">
            ...
            <code class="block break-all rounded bg-background px-2 py-1 select-all">{{
              oauthStatus?.device?.verification_uri
            }}</code>
          </div>
          <div class="space-y-1">
            ...
            <div class="flex items-center gap-2">
              <code
                class="block flex-1 rounded bg-background px-2 py-1 text-sm tracking-[0.3em] select-all"
                >{{ oauthStatus?.device?.user_code }}</code
              >
              <Button type="button" variant="ghost" size="sm" @click="handleCopyDeviceCode">
                <Copy />
                {{ $t("common.copy") }}
              </Button>
            </div>
          </div>
          <div v-if="oauthStatus?.device?.expires_at" class="text-muted-foreground">
        ...
        <div
          v-if="
            form.values.client_type === 'github-copilot' && oauthStatus?.has_token && !oauthExpired
          "
        ...
        >
          ...
          <div class="text-sm font-medium">
            {{
              oauthStatus?.account?.email ||
              oauthStatus?.account?.label ||
              oauthStatus?.account?.name ||
              oauthStatus?.account?.login ||
              $t("provider.oauth.status.authorizedCurrent")
            }}
          </div>
          <div
            v-if="
              [
                oauthStatus?.account?.login?.trim() ? `@${oauthStatus.account.login.trim()}` : '',
                oauthStatus?.account?.email?.trim() ?? '',
              ]
                .filter(Boolean)
                .join(' · ')
            "
        ...
        <div class="flex gap-2">
          ...
          >
            ...
            {{
              $t(
                form.values.client_type === "github-copilot"
                  ? "provider.oauth.deviceAuthorize"
    ...
    <section class="flex justify-between items-center mt-4">
      ...
      <div class="flex gap-4">
        ...
        >
          <template #trigger>
            <Button type="button" variant="outline" :aria-label="$t('common.delete')">
              <Trash2 />
            </Button>
          </template>
        </ConfirmPopover>
    ...
    <section v-if="testResult" class="mt-4 rounded-lg border p-4 space-y-3 text-xs">
      <div class="flex items-center gap-2">
        <StatusDot :status="testResult.reachable ? 'success' : 'error'" />
        <span class="font-medium">
          {{ testResult.reachable ? $t("provider.reachable") : $t("provider.unreachable") }}
        </span>
        <span v-if="testResult.latency_ms" class="text-muted-foreground">
          {{ testResult.latency_ms }}ms
        </span>
      </div>

      <div v-if="testResult.message" class="text-muted-foreground text-xs">
        {{ testResult.message }}
      </div>

      <div v-if="testError" class="text-destructive text-xs">
        {{ testError }}
      </div>
    </section>
  </form>
...
watch(
  () => [props.provider?.id, form.values.client_type] as const,
  async ([id, clientType]) => {
...
const hasChanges = computed(() => {
  ...
  const cacheChanged =
    supportsPromptCache(form.values.client_type) &&
