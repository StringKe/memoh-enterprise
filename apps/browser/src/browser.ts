...
export async function getOrCreateBotBrowser(
  botId: string,
  core: BrowserCore,
): Promise<BotBrowserEntry> {
  const existing = botBrowsers.get(botId);
  if (existing) {
    if (existing.core !== core) {
      // Reject core change if active sessions/contexts exist
      throw new Error(
        `Bot ${botId} already has a ${existing.core} browser. Cannot switch to ${core} while active.`,
      );
    }
    return existing;
  }
  ...
  const promise = (async () => {
    if (botBrowsers.size >= MAX_BOT_BROWSERS) {
      throw new Error(
        `Browser limit reached (${MAX_BOT_BROWSERS}). Cannot create new browser for bot ${botId}.`,
      );
    }
