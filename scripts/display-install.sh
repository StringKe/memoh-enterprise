...
install_debian() {
  ...
  apt-get update
  progress 42 installing "Installing VNC, Xfce, and fonts"
  apt-get install -y --no-install-recommends ca-certificates curl gnupg dbus-x11 x11-xserver-utils xterm xfce4 tigervnc-standalone-server fontconfig fonts-dejavu fonts-noto-color-emoji
...
install_alpine() {
  ...
  progress 42 installing "Installing VNC, Xfce, browser, and fonts"
  apk add --no-cache tigervnc xkeyboard-config xfce4 xfce4-terminal dbus-x11 xterm chromium fontconfig ttf-dejavu font-noto-emoji
}
