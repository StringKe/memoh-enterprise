  ...
  <div class="space-y-6 mr-4">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        >
          <ChannelIcon :channel="platformType" size="1.5em" />
        </span>
        <div>
      ...
      <div class="flex items-center gap-2">
        <template v-if="isEditMode">
          <Button variant="outline" size="sm" :disabled="isBusy" @click="handleToggleDisabled">
            <Spinner v-if="action === 'toggle'" class="mr-1.5" />
            {{
          ...
          >
            <template #trigger>
              <Button variant="destructive" size="sm" :disabled="isBusy">
                <Spinner v-if="action === 'delete'" class="mr-1.5" />
                {{ $t("common.delete") }}
              </Button>
            </template>
          </ConfirmPopover>
    ...
    <div v-if="showWebhookCallback" class="space-y-2">
      ...
      <template v-if="webhookCallbackUrl">
        <div class="flex gap-2">
          <Input :model-value="webhookCallbackUrl" readonly class="font-mono text-xs" />
          <Button variant="outline" size="sm" @click="copyWebhookCallback">
            {{ $t("common.copy") }}
          </Button>
        </div>
      </template>
    ...
    <div v-if="channelItem.meta.type === 'weixin'">
      <WeixinQrLogin :bot-id="botId" @login-success="handleWeixinLoginSuccess" />
    ...
    <div class="space-y-4">
      ...
      <p v-if="showWebhookCallback" class="text-xs text-muted-foreground">
      ...
      <div class="grid gap-4 md:grid-cols-2">
        ...
        >
          <Label
            :for="
              field.type === 'bool' || field.type === 'enum' ? undefined : `channel-field-${key}`
            "
          >
            {{ field.title || key }}
            <span v-if="!field.required" class="text-xs text-muted-foreground ml-1"
          ...
          <div v-if="field.type === 'secret'" class="relative">
            ...
            >
              <component :is="visibleSecrets[key] ? EyeOff : Eye" class="size-3.5" />
            </button>
          </div>
    ...
    <div class="flex justify-end gap-2">
      <template v-if="isEditMode">
        <Button :disabled="isBusy" @click="handleEditSave">
          <Spinner v-if="action === 'save'" class="mr-1.5" />
          {{ $t("common.save") }}
        </Button>
      </template>
      <template v-else>
        <Button variant="outline" :disabled="isBusy" @click="handleCreateSaveOnly">
          <Spinner v-if="action === 'save'" class="mr-1.5" />
          {{ $t("bots.channels.saveOnly") }}
        </Button>
        <Button :disabled="isBusy" @click="handleCreateSaveAndEnable">
          <Spinner v-if="action === 'save'" class="mr-1.5" />
...
function validateFeishuWebhookSecrets(): boolean {
  ...
  const encryptKey = String(
    form.credentials.encryptKey ?? form.credentials.encrypt_key ?? "",
  ).trim();
  const verificationToken = String(
    form.credentials.verificationToken ?? form.credentials.verification_token ?? "",
  ).trim();
...
async function saveChannel(disabled: boolean, nextAction: "save" | "toggle") {
  ...
  try {
    ...
    toast.error(
      detail ? `${t("bots.channels.saveFailed")}: ${detail}` : t("bots.channels.saveFailed"),
    );
  } finally {
    action.value = "";
  }
...
async function handleDelete() {
  ...
  try {
    ...
    const detail = err instanceof Error ? err.message : "";
    toast.error(
      detail ? `${t("bots.channels.deleteFailed")}: ${detail}` : t("bots.channels.deleteFailed"),
    );
  } finally {
    action.value = "";
  }
...
async function copyWebhookCallback() {
  ...
  try {
    ...
    const detail = err instanceof Error ? err.message : "";
    toast.error(
      detail ? `${t("bots.channels.copyFailed")}: ${detail}` : t("bots.channels.copyFailed"),
    );
  }
}
</script>
