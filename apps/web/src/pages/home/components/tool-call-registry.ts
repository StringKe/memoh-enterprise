...
function execErrorState(block: ToolCallBlock): { isError: boolean; suffix: string } {
  ...
  if (bg && block.done) {
    if (bg.status === "failed" || bg.status === "killed") {
      return {
        isError: true,
        suffix: typeof bg.exitCode === "number" ? `(exit ${bg.exitCode})` : "",
      };
    }
...
export function getToolDisplay(block: ToolCallBlock): ToolDisplay {
  ...
  switch (block.toolName) {
    ...
    case "list_schedule":
      return {
        icon: Calendar,
        actionKey: "list_schedule",
        target: "",
        detail: ToolCallDetailSchedule,
      };
    case "get_schedule":
    ...
    case "transcribe_audio": {
      const target = pickString(input, "path", "audio_path", "file_path", "url", "audio_url");
